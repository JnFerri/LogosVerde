...
const ProjectPage = () => {
  ...
  const shouldFetch: boolean =
    !selectedProject || selectedProject.id !== Number(projectId);
  const { data: projectData } = useProject(Number(projectId), shouldFetch);

  const shouldFetchAreaPlantings: boolean =
    !selectedProject?.plantingAreas ||
    selectedProject?.plantingAreas.length === 0;

  const { data: plantingAreasData } = usePlantingAreasByProjectId(
    selectedProject ? selectedProject?.id : projectData?.id,
    shouldFetchAreaPlantings,
  );
  ...
  return (
    ...
    >
      ...
      />
      <DashBoardProject
        selectedProject={selectedProject ? selectedProject : projectData}
      />
      ...
      >
        <CardsPlantingArea
          plantingAreas={
            selectedProject?.plantingAreas || plantingAreasData || []
          }
        />
      </Box>
    </Box>
