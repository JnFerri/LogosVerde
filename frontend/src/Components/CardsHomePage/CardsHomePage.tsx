...
const cards = [
  {
    ...
    description:
      "Crie um novo projeto onde poderá adiministrar suas aréas de plantio.",
  },
  {
    id: 1,
  ...
  return (
    <Box
      sx={{
        width: "100%",
        display: "grid",
        gridTemplateColumns: "repeat(auto-fit, minmax(200px,min(600px, 100%)))",
        gap: 2,
    ...
    >
      {cards.map((card) => (
        <Card key={card.id}>
          <CardActionArea
            onClick={() =>
              card.id === 0 ? isOpenModal() : navigate("/projects")
            }
            sx={{
              height: "100%",
              "&:hover": {
